...
async function filtrarDisponiveis(idsProfissionais, data, hora) {
  ...
  if (hora) {
    const [agendamentosDoDia] = await pool.query(
      `SELECT profissional_id, hora, duracao_estimada_minutos, duracao_minutos
       FROM agendamentos
       WHERE profissional_id IN (${placeholders}) AND data = ?
         AND status NOT IN ('cancelado')`,
      [...idsProfissionais, data]
    );

    const alvoMinutos = horaParaMinutos(hora);
    for (const ag of agendamentosDoDia) {
      const inicio = horaParaMinutos(ag.hora);
      const fim = inicio + duracaoOcupadaMinutos(ag);
