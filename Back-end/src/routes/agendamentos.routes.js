...
// SUBMETER UMA AVALIAÇÃO (depois do cliente pagar) — fica pendente até um
// gestor/admin aprovar no backoffice
router.post("/pagamento/:token/avaliacao", async (req, res) => {
  try {
    const { token } = req.params;
    ...
    if (!Number.isInteger(notaNumero) || notaNumero < 1 || notaNumero > 5) {
      return res.status(400).json({ mensagem: "A nota deve ser um número inteiro de 1 a 5" });
    }

    const [[agendamento]] = await pool.query(
      "SELECT id, profissional_id, nome_cliente FROM agendamentos WHERE token_pagamento = ?",
      [token]
    );
    ...
    if (!agendamento.profissional_id) {
      return res.status(409).json({ mensagem: "Este serviço não tem um profissional associado" });
    }

    const [[jaAvaliado]] = await pool.query(
      "SELECT id FROM avaliacoes WHERE agendamento_id = ?",
      [agendamento.id]
    );
    if (jaAvaliado) {
      return res.status(409).json({ mensagem: "Este serviço já foi avaliado" });
    }

    await pool.query(
      `INSERT INTO avaliacoes (agendamento_id, profissional_id, nome_cliente, nota, comentario, status)
       VALUES (?, ?, ?, ?, ?, 'pendente')`,
      [agendamento.id, agendamento.profissional_id, agendamento.nome_cliente, notaNumero, comentario || null]
    );

    res.status(201).json({ mensagem: "Obrigado pela sua avaliação! Ela será publicada após revisão." });
  } catch (erro) {
    console.error(erro);
    res.status(500).json({ mensagem: "Erro ao submeter avaliação" });
  }
});
